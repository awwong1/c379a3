...
int main(int ac, char *av[])
{
	...
	if ( ac != 2 ){
		printf("usage: saucer num_active_enemies ..\n"); 
		exit(1);
	}

	num_msg = setup_saucers(av+1,props);
...
int setup_saucers(char *strings[], struct propset props[])
{
	int tnum_msg = (int) strtol(*strings, NULL, 10);
	int num_msg = ( tnum_msg > MAXMSG ? MAXMSG : tnum_msg);
	int i;
